...
const seedData = async () => {
  try {
    ...
    const borrowerUser = await User.create({ name: 'John Borrower', email: 'john@example.com', password: 'borrow123', role: 'borrower' });
    ...
    console.log(`Created ${authors.length} authors, ${books.length} books, ${borrowers.length} borrowers, ${transactions.length} transactions`);
